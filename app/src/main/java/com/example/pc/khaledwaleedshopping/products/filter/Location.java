...
import static com.example.pc.khaledwaleedshopping.products.filter.FullFilter.govTitle;
    ...
    public Location(@NonNull final FragmentActivity activity, final String kind, String govId) {
        ...
        if (kind.equals("gov")) {
            ...
            try {
                ...
                String any = "{\"" + "name" + "\": \"Whole country\" , \"id\": \"-1\"}";
                JSONObject object = new JSONObject(any);
                jsonObjects.add(object);
                for (int i = 0; i < FullFilter.jsonObject.getJSONArray("government").length(); i++)
                    jsonObjects.add(FullFilter.jsonObject.getJSONArray("government").getJSONObject(i));
            ...
            try {
                title.setText(govTitle);
                boolean find = false;
                List<JSONObject> jsonObjects = new ArrayList<>();
                String any = "{\"" + "name" + "\": \"All\" , \"id\": \"-1\"}";
                JSONObject object = new JSONObject(any);
                jsonObjects.add(object);
                for (int i = 0; i < jsonObject.getJSONObject("city").getJSONArray(govId).length(); i++) {
                    jsonObjects.add(jsonObject.getJSONObject("city").getJSONArray(govId).getJSONObject(i));
                    find = true;
                }
            ...
            try {
                ...
                holder.customeTextView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        try {
                            if (kind.equals("gov")) {
                                FullFilter.govTitle = jsonArray.get(position).getString("name");
                                FullFilter.city_id = "-1";
                                if (jsonArray.get(position).getString("id").equals("-1")) {
                                    FullFilter.location.setText(jsonArray.get(position).getString("name"));
                                } else
                                    new Location(activity, "city", jsonArray.get(position).getString("id"));
                            } else if (kind.equals("city")) {
                                FullFilter.government_id = govId;
                                FullFilter.city_id = jsonArray.get(position).getString("id");
                                FullFilter.location.setText(jsonArray.get(position).getString("name"));
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
